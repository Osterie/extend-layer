...
class CommandPromptOpener extends HotkeyAction {
    ...
    OpenToDefaultPath() {
        try {
            ...
            msgbox(
                "Error opening command prompt the given default path, perhaps it does not exist. `nThe default path you gave was: " .
                this.GetDefaultPath(), "Notify")
        }
    }

    SetDefaultPathFromFile() {
        commandPromptDefaultPath := this.IniFileReader.readOrCreateLine(FilePaths.GetPathToCurrentSettings(),
        "CommandPrompt", "DefaultPath", '"\C:\Users\"')
